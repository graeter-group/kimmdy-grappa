import logging
import numpy as np

np.set_printoptions(suppress=True, precision=6)  # prevent numpy exponential
import math
from typing import Union

from kimmdy.plugins import Parameterizer
from kimmdy.topology.topology import Topology


from grappa.utils.kimmdy_utils import KimmdyGrappaParameterizer
from grappa.utils.loading_utils import model_from_tag

logger = logging.getLogger("kimmdy.grappa_interface")


class GrappaInterface(Parameterizer):
    '''
    Wrapper of the KimmdyGrappaParameterizer used in grappa. Initialised with a tag instead of a model.
    '''
    def __init__(self, *args, grappa_tag:str='latest', charge_model:str='classical', **kwargs):
        super().__init__(*args, **kwargs)
        grappa_model = model_from_tag(grappa_tag)
        self.kimmdy_grappa_parameterizer = KimmdyGrappaParameterizer(grappa_model=grappa_model, charge_model=charge_model)

    def parameterize_topology(
            self, current_topology: Topology, focus_nr: list[str] = []
        ) -> Topology:
        return self.kimmdy_grappa_parameterizer.parameterize(current_topology=current_topology, focus_nr=focus_nr)